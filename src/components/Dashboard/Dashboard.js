...
class Dashboard extends Component {
    ...
    render() { 
    const elements = this.state.habits.map((habit, index) => (<li key={index}>{habit.name}</li>))
        return (
            <div className="container">
                ...
                <div className="row">
                    <div className="col-sm-4 m-auto">
                        <button className="btn btn-success" onClick={this.handleShowButtonClick}>Create New Habit</button>
                    </div>
                </div>
                {this.state.showCreateHabit ? 
                (<div className="row">
                    <HabitForm email={this.state.email} onHabitAdd={this.handleFormSubmit}/>
                </div>) :
                ''
                }
